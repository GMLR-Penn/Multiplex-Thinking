...
class DataProto:
    ...
    def subset(self, indices):
        """
        Subset the batch data using the given indices.

        Args:
            indices (torch.Tensor): Indices to subset the batch data.

        Returns:
            DataProto: A new DataProto with the subsetted data.
        """
        indices_np = indices.detach().cpu().numpy()
        ret = copy.deepcopy(self)
        ret.batch = self.batch[indices]
        ret.non_tensor_batch = {key: val[indices_np] for key, val in self.non_tensor_batch.items()}
        return ret
    
    def shuffle(self):
        """
        Shuffle the batch data in-place.
        """
        indices = torch.randperm(len(self.batch))
        self.reorder(indices)
    
    def set_item(self, item, data):
        assert isinstance(data, DataProtoItem)
        # print(f"self.batch[item]['indices_before_dispatch']: {self.batch[item]['indices_before_dispatch']}")
        # print(f"data.batch['indices_before_dispatch']: {data.batch['indices_before_dispatch']}")
        # print(f"self.batch[item]: {self.batch[item]}")
        # print(f"data.batch: {data.batch}", flush=True)
        self.batch[item] = data.batch.clone()
        for key, val in data.non_tensor_batch.items():
            if key in self.non_tensor_batch:
                self.non_tensor_batch[key][item] = copy.deepcopy(val)
            else:
                raise ValueError(f"Key {key} not found in non_tensor_batch")
