...
def ulysses_pad_and_slice_inputs_for_topk_probs_and_indices(topk_probs_rmpad: torch.Tensor, topk_indices_rmpad: torch.Tensor, position_ids_rmpad: Optional[torch.Tensor] = None, sp_size: int = 1):
    """
    Pad and slice 3D embeddings tensor to be divisible by sp_size.
    Only operates on the sequence dimension (dim=1), preserving the last dimension.
    
    Args:
        embeddings_rmpad: shape of [bsz, seqlen, hidden_size] or [bsz, seqlen, topk]
        position_ids_rmpad: shape of [bsz, seqlen], where bsz must be 1
        sp_size (int): ulysses sequence parallelism size

    Returns:
        torch.Tensor: padded and sliced embeddings
        torch.Tensor: padded and sliced position_ids  
        int: pad size
    """

    topk_probs_rmpad, topk_indices_rmpad, position_ids_rmpad, pad_size = ulysses_pad_for_topk_probs_and_indices(topk_probs_rmpad, topk_indices_rmpad, position_ids_rmpad, sp_size)
    topk_probs_rmpad = slice_input_tensor(topk_probs_rmpad, dim=1, padding=False)
    topk_indices_rmpad = slice_input_tensor(topk_indices_rmpad, dim=1, padding=False)
    if position_ids_rmpad is not None:
        position_ids_rmpad = slice_input_tensor(position_ids_rmpad, dim=1, padding=False)
    return topk_probs_rmpad, topk_indices_rmpad, position_ids_rmpad, pad_size
