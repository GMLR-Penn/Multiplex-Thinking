...
def compute_multiplex_thinking_policy_loss(
    ...
    """
    Compute PPO policy loss for soft thinking mode.
    This uses the same clipping logic as standard PPO but works with topk-weighted log probs.
    
    Args:
        old_log_prob (torch.Tensor): Old policy weighted log-probs, shape (batch_size, response_length, topk)
        log_prob (torch.Tensor): Current policy weighted log-probs, shape (batch_size, response_length, topk) 
        advantages (torch.Tensor): Advantage estimates, shape (batch_size, response_length)
        response_mask (torch.Tensor): Valid token mask, shape (batch_size, response_length)
        topk_probs (torch.Tensor): Topk probabilities, shape (batch_size, response_length, topk)
        
    Returns:
        Same as compute_policy_loss: (pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower)
    """
    response_mask_3d = response_mask.unsqueeze(-1).repeat(1, 1, log_prob.shape[-1])

    # Detect answer tokens: [1.0, 0., 0., ...]
    is_answer_token = (topk_probs[:, :, 0] == 1.0) & (topk_probs[:, :, 1:].sum(dim=-1) == 0.0)  # (bsz, response_length)
    
    # Debug: print answer token statistics
    num_answer_tokens_per_seq = is_answer_token.sum(dim=-1)  # (bsz,) - number of answer tokens in each sequence
    total_tokens_per_seq = response_mask.sum(dim=-1)  # (bsz,) - total valid tokens in each sequence
    answer_token_ratio = (num_answer_tokens_per_seq.float() / total_tokens_per_seq.float().clamp(min=1.0)).mean()  # Aggregate to scalar

    
    # For answer tokens: mask out weight=0 positions; For thinking tokens: keep all positions
    topk_weight_mask = topk_probs > 1e-20  # (bsz, response_length, topk)
    # Apply mask only to answer tokens
    topk_weight_mask = torch.where(
        is_answer_token.unsqueeze(-1),  # if answer token
        topk_weight_mask,                # use weight mask (keep only weight=1.0 position)
        torch.ones_like(topk_weight_mask)  # else keep all positions for thinking tokens
    )
    response_mask_3d = response_mask_3d & topk_weight_mask
    
        
    negative_approx_kl = log_prob - old_log_prob # (bsz, response_length, topk) 
    negative_approx_kl = torch.clamp(negative_approx_kl, min=-20.0, max=20.0)
    ratio = torch.exp(negative_approx_kl) # (bsz, response_length, topk)
    
    ppo_kl = verl_F.masked_mean(-negative_approx_kl, response_mask_3d) # (bsz, response_length, topk)
    
    advantages_3d = advantages.detach().unsqueeze(-1).repeat(1, 1, log_prob.shape[-1])
    
    pg_losses1 = -advantages_3d * ratio  # (bsz, response_length, topk)
    ...
    pg_losses2 = -advantages_3d * torch.clamp(ratio, 1 - cliprange_low, 1 + cliprange_high) # (bsz, response_length, topk)
    clip_pg_losses1 = torch.maximum(pg_losses1, pg_losses2) # max(-ratio * A, -clip(ratio, 1-cliprange, 1+cliprange) * A)
    pg_clipfrac = verl_F.masked_mean(torch.gt(pg_losses2, pg_losses1).float(), response_mask_3d) # (bsz, response_length, topk)
    pg_losses3 = -advantages_3d * clip_ratio_c # (bsz, response_length, topk)
    clip_pg_losses2 = torch.min(pg_losses3, clip_pg_losses1) # (bsz, response_length, topk)
    pg_clipfrac_lower = verl_F.masked_mean(torch.gt(clip_pg_losses1, pg_losses3) * (advantages_3d < 0).float(), response_mask_3d) # (bsz, response_length, topk)
    pg_losses = torch.where(advantages_3d < 0, clip_pg_losses2, clip_pg_losses1) # (bsz, response_length, topk)
    
    
    pg_losses_summed = (pg_losses * response_mask_3d.float()).sum(dim=-1)
    
    pg_loss = agg_loss(loss_mat=pg_losses_summed, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)
    
    # logprob_mask_ratio = torch.sum(response_mask_3d).item() / torch.sum(response_mask_3d).item()
