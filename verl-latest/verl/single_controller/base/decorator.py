...
def init_predefined_dispatch_mode():
    ...
    Dispatch.register("SORT_DP_COMPUTE_PROTO")
...
def _shuffle_args_kwargs_data_proto(chunks, *args, **kwargs):
    from verl.protocol import DataProto, DataProtoFuture
    from copy import deepcopy

    # this function do in-place shuffle so we copy the args and kwargs before shuffle
    args = deepcopy(args)
    kwargs = deepcopy(kwargs)

    splitted_args = []
    # NOTE current implementation assumes that all args and kwargs with same lengths
    data_length = None
    indices_before_dispatch = None
    index_key = "indices_before_dispatch"
    for arg in args:
        assert isinstance(arg, (DataProto))
        assert index_key not in arg.batch, f"expecting {arg} to not have {index_key} in arg.batch"
        if data_length is None:
            data_length = len(arg)
            indices_before_dispatch = torch.arange(data_length, dtype=torch.long)
        else:
            assert data_length == len(arg), f"expecting {data_length} == {len(arg)}"
        arg.batch[index_key] = indices_before_dispatch
        # shuffle the data proto
        # TODO debug remove
        # print(f"before shuffle {arg.batch["indices_before_dispatch"]}")
        arg.shuffle()
        # print(f"after shuffle {arg.batch["indices_before_dispatch"]}\n\n", flush=True)
        splitted_args.append(arg.chunk(chunks=chunks))

    splitted_kwargs = {}
    for key, val in kwargs.items():
        assert isinstance(val, (DataProto))
        assert index_key not in val.batch, f"expecting {val} to not have {index_key} in val.batch"
        if data_length is None:
            data_length = len(val)
            indices_before_dispatch = torch.arange(data_length, dtype=torch.long)
        else:
            assert data_length == len(val), f"expecting {data_length} == {len(val)}"
        val.batch[index_key] = indices_before_dispatch
        # print(f"before shuffle {val.batch["indices_before_dispatch"]}")
        val.shuffle()
        # print(f"after shuffle {val.batch["indices_before_dispatch"]}\n\n", flush=True)
        splitted_kwargs[key] = val.chunk(chunks=chunks)

    return splitted_args, splitted_kwargs


def _sort_data_proto_to_original_order(data_proto):
    from verl.protocol import DataProto, DataProtoFuture
    import ray

    if isinstance(data_proto, DataProto):
        original_indices = data_proto.batch.pop("indices_before_dispatch")
        reordered_indices = torch.argsort(original_indices)
        data_proto.reorder(reordered_indices)

    elif isinstance(data_proto, DataProtoFuture):
        raise NotImplementedError
    else:
        raise RuntimeError(f"expecting {data_proto} to be DataProto or DataProtoFuture, but got {type(data_proto)}")


def _sort_args_kwargs_data_proto(*args, **kwargs):
    import torch
    from verl.protocol import DataProto, DataProtoFuture
    sorted_args = None
    key_args_kwargs = []
    # assert len(args) == 1 and len(kwargs) == 0, f"We currently only support one arg and no kwargs, but got {args} and {kwargs}"
    # data = args[0]
    # assert isinstance(data, (DataProto, DataProtoFuture)), f"expecting {data} to be DataProto, but got {type(data)}"
    assert len(kwargs) == 0, f"We currently only *args"
    key_and_indices = []
    sorted_indices = None
    for data_idx, data in enumerate(args):
        assert isinstance(data, (DataProto)), f"expecting {data} to be DataProto, but got {type(data)}"
        if data_idx == 0:
            for i in range(len(data)):
                key_and_indices.append((data.non_tensor_batch['uid'][i], i))
            key_and_indices.sort(key=lambda x: x[0])
            sorted_indices = torch.tensor([x[1] for x in key_and_indices], dtype=torch.long)
        else:
            assert len(data) == len(args[0]), f"expecting {len(data)} == {len(args[0])}"
...
def dispatch_sort_dp_compute_data_proto(worker_group, *args, **kwargs):
    from verl.single_controller.base.worker_group import WorkerGroup
    assert isinstance(worker_group, WorkerGroup)
    sorted_args, sorted_kwargs = _sort_args_kwargs_data_proto(*args, **kwargs)
    splitted_args, splitted_kwargs = _split_args_kwargs_data_proto(worker_group.world_size, *sorted_args, **sorted_kwargs)
    return splitted_args, splitted_kwargs
...
DISPATCH_MODE_FN_REGISTRY = {
    ...
    Dispatch.SORT_DP_COMPUTE_PROTO: {
        'dispatch_fn': dispatch_sort_dp_compute_data_proto,
        'collect_fn': collect_dp_compute_data_proto
    },
}
