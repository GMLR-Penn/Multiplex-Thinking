...
class SamplingBatchInfo:
    ...
    def from_schedule_batch(cls, batch: ScheduleBatch, vocab_size: int):
        ...
        if enable_soft_thinking:
            # # Handle None values in soft_thinking_mode
            soft_thinking_modes = torch.tensor([
                req.sampling_params.soft_thinking_mode if req.sampling_params.soft_thinking_mode is not None else False 
                for req in reqs
            ], dtype=torch.bool).to(device, non_blocking=True)
            max_topk = batch.max_topk
            used_topk = batch.used_topk
            enable_entropy_mask = batch.enable_entropy_mask
            entropy_mask_threshold = batch.entropy_mask_threshold
            early_stopping_entropy_threshold = batch.early_stopping_entropy_threshold
            early_stopping_length_threshold = batch.early_stopping_length_threshold
            enable_gumbel = batch.enable_gumbel
            enable_max_topk = batch.enable_max_topk
            gumbel_tau = batch.gumbel_tau
            enable_replacement = batch.enable_replacement
            enable_gumbel_after_thinking = batch.enable_gumbel_after_thinking
            enable_unweighting = batch.enable_unweighting
            dirichlet_alpha = batch.dirichlet_alpha
            batch_size = len(reqs)
            after_thinking_temperature = batch.after_thinking_temperature
            after_thinking_top_p = batch.after_thinking_top_p
            after_thinking_top_k = batch.after_thinking_top_k
            after_thinking_min_p = batch.after_thinking_min_p
        else:
            soft_thinking_modes = None
            max_topk = None
            used_topk = None
            enable_entropy_mask = None
            entropy_mask_threshold = None
            early_stopping_entropy_threshold = None
            early_stopping_length_threshold = None
            after_thinking_temperature   = None
            after_thinking_top_p = None
            after_thinking_top_k = None
            after_thinking_min_p = None
            dirichlet_alpha = None
            enable_gumbel = None
            enable_max_topk = None
            gumbel_tau = None
            enable_replacement = None
            enable_gumbel_after_thinking = None
            enable_unweighting = None
        ...
        ret = cls(
            ...
            after_thinking_top_k=after_thinking_top_k,
            after_thinking_min_p=after_thinking_min_p,
            enable_soft_thinking=batch.enable_soft_thinking,
            soft_thinking_modes=soft_thinking_modes,
            max_topk=max_topk,
            used_topk=used_topk,
            enable_entropy_mask=enable_entropy_mask,
            entropy_mask_threshold=entropy_mask_threshold,
            ...
            enable_max_topk=enable_max_topk,
            gumbel_tau=gumbel_tau,
            enable_replacement=enable_replacement,
            enable_gumbel_after_thinking=enable_gumbel_after_thinking,
